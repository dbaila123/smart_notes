  ...
  END    
   ...
   from Cargos    
    ...
    if (select count(sConfiguration) from @configurations_request where sConfiguration = 'lideres_directores') = 1    
        ...
        p.sPersona_Nombre as sDescripcion,    
                   CASE    
                       WHEN r.sDescripcion like '%LÍDER%' THEN 'LIDER'    
        ...
        BEGIN    
            declare @isQATeam int = (SELECT count(*)    
                                     ...
                                     where nId_Proyecto = (select nId_Proyecto    
                                                           from Proyectos p    
                                                 where sNombre = 'PRY - ASEGURAMIENTO DE LA CALIDAD')    
        ...
        BEGIN    
            ...
            SELECT p.nId_Proyecto   AS sCodigo,    
                   CONCAT(p.sNombre, ' ', '(',    
                          (SELECT TOP 1 value    
                           FROM (SELECT ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS row, value    
                                 FROM STRING_SPLIT(p_coor.sPersona_Nombre, ' ')) AS nombres    
                           ORDER BY row DESC),    
                          ')')      AS sDescripcion,    
        ...
        BEGIN    
            ...
            from ((select p.nId_Proyecto                                        as sCodigo,    
                          CONCAT(p.sNombre, ' ', '(', (SELECT TOP 1 value    
                                                       from (SELECT ROW_NUMBER() OVER (ORDER BY (select null)) as row, value    
       FROM STRING_SPLIT(p2.sPersona_Nombre, ' ')) as nombres    
           order by row desc), ')') as sDescripcion,    
 ...
 'PROYECTOS'                   as sTipo_Configuracion    
  ...
  (select p.nId_Proyecto                       as sCodigo,    
    ...
    IF (SELECT COUNT(sConfiguration) FROM @configurations_request WHERE sConfiguration = 'proyectos_colaborador_v2') = 1    
        BEGIN    
            ...
            SELECT p.nId_Proyecto                       as sCodigo,    
                   CONCAT(p.sNombre, ' ', '(', (SELECT TOP 1 value    
                                                from (SELECT ROW_NUMBER() OVER (ORDER BY (select null)) as row, value    
                                                      FROM STRING_SPLIT(pc.sPersona_Nombre, ' ')) as nombres    
                                                order by row desc), ')') as sDescripcion,    
    ...
    IF (SELECT COUNT(sConfiguration)    
        ...
        BEGIN    
            INSERT INTO @configuraciones              SELECT r.nId_Requerimiento          as sCodigo,                         r.sNombre                     as sDescripcion,    
    ...
    IF (select count(sConfiguration)    
        ...
        BEGIN    
            ...
            from Requerimientos r    
                     ...
                     join Personas p_sup on p_sup.nId_Persona = supervisor.nId_Persona    
                     join Usuarios u_sup on u_sup.nId_Persona = p_sup.nId_Persona    
       where u_sup.nId_Usuario = @id_usuario            END    

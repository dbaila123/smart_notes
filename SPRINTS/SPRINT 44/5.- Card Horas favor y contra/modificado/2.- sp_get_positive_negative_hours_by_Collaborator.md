```sql
CREATE OR ALTER PROCEDURE [dbo].[sp_get_positive_negative_hours_by_Collaborator](    
    @dFecha DATETIME,    
...
USING (
	SELECT
		COALESCE(ta.nId_Colaborador, t.nId_Colaborador, ea.nId_Colaborador, bs.nId_Colaborador) AS nId_Colaborador,
        ...
        ISNULL(ta.nAFavorApro, 0) AS nTotal_Min_Favor_CompRecu_Apro,
        ...
        ISNULL(ta.nAFavorAP, 0) + ISNULL(ea.nExtraApro, 0) + ISNULL(ea.nExtraPend, 0) AS nTotal_Favor,
        ISNULL(t.dTardanza_Con_Tolerancia,0)+ ISNULL(ta.nPermisosApro, 0)- ISNULL(ta.nAFavorAP,0) AS nTotal_Minutos_Recuperar
    FROM @tTardanzas t
    FULL OUTER JOIN dbo.fn_get_request_hours_by_Transaction_All(@sIds_Colaborador, 0, 0, 0, NULL, CONVERT(DATE, GETDATE())) ta ON t.nId_Colaborador = ta.nId_Colaborador
    FULL OUTER JOIN @tExtra ea ON ea.nId_Colaborador = ta.nId_Colaborador
    JOIN Colaboradores col ON col.nId_Colaborador = ta.nId_Colaborador
    JOIN Personas per ON per.nId_Persona = col.nId_Persona
    FULL OUTER JOIN @tBolsa bs ON bs.nId_Colaborador = ta.nId_Colaborador

...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            BEGIN
                -- Verificar que la transacción no haya sido ya procesada por el otro trigger
                IF NOT EXISTS (
                    SELECT 1
                    FROM Transacciones_Saldo_Mins 
                    WHERE nId_Entidad = @nId_Entidad
                    AND nId_Tipo_Entidad = 2
                    AND sObservacion LIKE '%[Procesado por trigger Solicitudes]%'
                )
                BEGIN

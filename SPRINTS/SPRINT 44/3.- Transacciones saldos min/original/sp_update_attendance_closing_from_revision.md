...
BEGIN  
 ...
 DECLARE @Request_Create TABLE (nId_Solicitud INT, nId_Colaborador INT  
, nCantidad INT);  
 DECLARE @nIdSolicitudes NVARCHAR(MAX);  
 DECLARE @nIdColaborador NVARCHAR(MAX);  
 DECLARE @nCantidad NVARCHAR(MAX);  
  
 SET @Motivo = N'ACUMULACIÓN DE TARDANZAS EN TOLERANCIA DEL CIERRE GENERADO EL DÍA ' + CONVERT(NVARCHAR(10), @ActualDate, 103);  
 ...
 IF @nEstado_Cierre = 4 --EN REVISI�N  
 BEGIN  
    ...
    FROM @ListaItems;  
     ...
     -- ACTUALIZAR TARDANZAS POR COLABORADOR  
               ...
               -- INSERTANDO NUEVA TRANSACCI�N PARA RESTAR LA TARDANZA DEL PULL DE HORAS  

...
@nid_ApprovedReqId int,

@nid_PayReqId int

  

select @nid_ApprovedReqId = sCodigo from Configs where sTabla = 'ESTADO_SOLICITUD' and sCodigo_Externo = 'A' -- 3

select @nid_PayReqId = sCodigo from Configs where sTabla = 'ESTADO_SOLICITUD' and sCodigo_Externo = 'PAG' -- 9
...
(s.nId_Tipo_Solicitud = @nid_PayReqId AND s.nEstado_Solicitud = @nid_ApprovedReqId) OR
...
WHEN (s.nId_Tipo_Solicitud = @nid_PayReqId AND s.nEstado_Solicitud = @nid_ApprovedReqId) THEN '[]'
...
JOIN Solicitudes s ON tsm.nId_Entidad = s.nId_Solictud and cast (s.nEstado_Solicitud as int) = @nid_ApprovedReqId
...
tsm.nId_Cierre IS NULL and

s.dFecha_Inicio <= @dFecha_Fin and

tsm.nId_Sub_Tipo_Entidad = 6

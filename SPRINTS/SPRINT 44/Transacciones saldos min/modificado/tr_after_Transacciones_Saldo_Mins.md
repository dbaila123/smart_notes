...
WHERE EXISTS (  
    ...
    WHERE Transacciones_Saldo_Mins.nId_Entidad =   
            CASE   
                WHEN i.nId_Tipo_Solicitud = 8 THEN i.nId_Tarea  
                ELSE i.nId_Solictud  
            END  
        AND Transacciones_Saldo_Mins.nId_Tipo_Entidad =   
            CASE   
                WHEN i.nId_Tipo_Solicitud = 8 THEN 9  
                ELSE 1  
            END  
);  
  ...
  END AS nId_Tipo_Entidad,  
        CASE  
            WHEN i.nId_Tipo_Solicitud = 8 THEN 8  
  ...
  END AS nId_Entidad,  
        ...
        i.sMotivo,  
  CASE  
            WHEN i.nId_Tipo_Solicitud in(1,26,8) THEN i.nCantidad  -- Usar el valor de dCantidad_Minutos  
        ...
        i.nUsuario_Update,
		CASE 
			WHEN i.nId_Tipo_Solicitud = 8 THEN i.nEstado_Solicitud
			ELSE NULL
		END AS nEstado_Entidad
    FROM inserted i  
    WHERE (  
 --Acepta solicitudes con tipo = 8 (recuperacion) que sean aprobadas y pendientes  
            (i.nId_Tipo_Solicitud IN (1, 2, 26) AND i.nEstado_Solicitud = 3)  
            OR  
            (i.nId_Tipo_Solicitud = 8 AND i.nEstado_Solicitud IN (1, 3))  
        )  
 AND NOT EXISTS (  
        SELECT 1  

...
SELECT  
  ...
  u_owner.sPersona_Nombre sNombre_Colaborador,
   CASE 
        WHEN docs.sTipo_Documento = 'PASAPORTE' THEN 'PAS'
        ELSE docs.sTipo_Documento
...
OUTER APPLY (
    ...
    INNER JOIN Personas p ON c.nId_Persona = p.nId_Persona 
    INNER JOIN Documentos d ON p.nId_Persona = d.nId_Persona
    INNER JOIN Configs con ON d.nTipo_Documento = con.sCodigo 
        AND con.sTabla = 'TIPO_DOCUMENTO'
    WHERE c.nId_Colaborador = tb_soli.nId_Colaborador
) docs
--------- Owner  

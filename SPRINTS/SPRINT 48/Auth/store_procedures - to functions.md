```sql
create or replace function fn_get_email_login(par_semail text) returns SETOF user_login  
    language plpgsql  
as  
$$  
DECLARE  
...
BEGIN  
    ...
    IF var_Colaborador > 0 THEN  
        RETURN QUERY            SELECT                u.nid_usuario,  
                c.nid_colaborador,  
...
--corregir
create or replace function fn_smart_get_email_login_super(par_semail text) returns user_login  
    language plpgsql  
as  
$$  
DECLARE  

```sql
create or replace function fn_get_configurations(p_configurations_string text, p_user_id integer, p_project_id integer, p_requirement_id integer, p_slug_listado text, p_collaborator_id integer DEFAULT 0)  
...
begin  
    ...
    if 'coordinadores_supervisor' = any(var_configuration_request) then  
        RETURN QUERY        SELECT            c.nid_coordinador::text,  
            p.spersona_nombre,  
    ...
    if 'especialidades' = any(var_configuration_request) then  
        RETURN QUERY        SELECT            e.nid_especialidad::text,  
            e.sdescripcion,  
            'ESPECIALIDADES'  
    ...
    if 'ocupaciones' = any(var_configuration_request) then  
        RETURN QUERY        SELECT            nid_ocupacion::text,  
            sdescripcion,  
            'OCUPACIONES'  
        FROM ocupaciones  
        WHERE nestado = 1;  
    ...
    if 'nacionalidades' = any(var_configuration_request) then  
        RETURN QUERY        SELECT            nid_pais::text,  
            sgentilicio,  
            'NACIONALIDADES'  
        FROM paises  
        WHERE nestado = 1  
    ...
    if 'areas' = any(var_configuration_request) then  
        RETURN QUERY        SELECT            a.nid_area::text,  
            a.sdescripcion,  
            'AREAS'  
        FROM areas a  
        WHERE nestado = 1;  
    ...
    if 'empresas' = any(var_configuration_request) then  
        RETURN QUERY        SELECT            eu.nid_empresa::text,  
            p.spersona_nombre,  
            'EMPRESAS'  
    ...
    if 'cargos' = any(var_configuration_request) then  
        RETURN QUERY        SELECT            c.nid_cargo::text,  
            c.sdescripcion,  
            'CARGOS'  
        FROM cargos c  
    ...
    if 'codigos_telefono_pais' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                p.nid_pais::text,  
    ...
    if 'ubigeos' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                u.nid_ubigeo::text,  
    ...
    if 'sedes' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                nid_sede::text,  
                sdescripcion,  
                'SEDES'  
            FROM sedes  
    ...
    if 'colaboradores_con_usuario' = any(var_configuration_request) then  
        RETURN QUERY            SELECT DISTINCT                c.nid_colaborador::text,  
                p.spersona_nombre,  
    ...
    if 'colaboradores' = any(var_configuration_request) then  
        ...
        if p_slug_listado = 'EQUIPO' then  
            ...
            RETURN QUERY  
            SELECT DISTINCT                tc.nid_colaborador::text,  
                pc.spersona_nombre,  
                'COLABORADORES'  
    ...
    if 'colaboradores_suggest' = any(var_configuration_request) then  
        RETURN QUERY        SELECT            collaborator_suggests_by_slug.scodigo,  
            collaborator_suggests_by_slug.sdescription,  
            collaborator_suggests_by_slug.stipo_configuration  
    ...
    if 'clientes' = any(var_configuration_request) then  
        RETURN QUERY        SELECT            c.nid_cliente::text,  
            p.spersona_nombre,  
            'CLIENTES'  
        FROM clientes c  
        LEFT JOIN personas p on c.nid_persona = p.nid_persona  
    ...
    if 'horarios' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                h.nid_horario::text,  
                h.sdescripcion,  
                'HORARIO'  
            FROM horarios h  
            WHERE h.nestado = 1;  
    ...
    if 'tipos_rol_interno_externo' = any(var_configuration_request) then  
        RETURN QUERY        SELECT            r.nid_rol::text,  
            r.sdescripcion,  
            CASE  
    ...
    if 'personas_sin_usuario' = any(var_configuration_request) then  
        RETURN QUERY        SELECT            p.nid_persona::text,  
            p.spersona_nombre,  
            CASE  
                WHEN (  
                    SELECT count(*)  
    ...
    if 'categoria_tarea' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                c.nid_categoria::text,  
                c.snombre,  
                'CATEGORIA_TAREA'  
            FROM categorias c  
            WHERE c.nestado = 1;  
    ...
    if 'proyectos_supervisor' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                pry.nid_proyecto::text,  
                pry.sdescripcion,  
    ...
    if 'proyectos_coor' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                p.nid_proyecto::text,  
                CONCAT(p.sNombre, ' ', '(',  
                       p2.sPrimer_Nombre ,  
                       ')'),  
                'PROYECTOS_COOR'  
    ...
    if 'proyectos_todos_masivo' = any(var_configuration_request) then  
        RETURN QUERY            select                scodigo::text,  
                sdescripcion,  
                stipo_configuracion  
    ...
    if 'proyectos' = any(var_configuration_request) then  
        RETURN QUERY            select                proyecto_codigo::text,  
    ...
    if 'requerimientos_por_proyecto' = any(var_configuration_request) then  
        RETURN QUERY            SELECT r.nid_requerimiento::text,  
                   r.snombre,  
    ...
    if 'proyectos_en_equipo_proyecto' = any(var_configuration_request) then  
        RETURN QUERY            SELECT DISTINCT p.nid_proyecto::text,  
                            CONCAT(p.snombre, ' ', '(', p2.sprimer_nombre, ')'),  
                            'PROYECTOS_EN_EQUIPO_PROYECTO'  
            FROM proyectos p  
            JOIN proyecto_lider pl ON p.nid_proyecto = pl.nid_proyecto AND pl.nestado = 1 --PRY_LIDER  
            -- coordinator name            JOIN coordinadores_proyectos cp ON p.nid_proyecto = cp.nid_proyecto  
            JOIN coordinadores c ON cp.nid_coordinador = c.nid_coordinador  
            JOIN personas p2 ON c.nid_persona = p2.nid_persona  
    ...
    if 'tipo_motivo' = any(var_configuration_request) then  
        RETURN QUERY            SELECT nId_Tipo_Motivo::text,  
    ...
    if 'frontend_version' = any(var_configuration_request) then  
        RETURN QUERY        SELECT            nid_v::text,  
            sversion::text,  
            'FRONTEND_VERSION'  
        FROM version  
        WHERE nestado = 1  
    ...
    if 'usuarios' = any(var_configuration_request) then  
        RETURN QUERY            SELECT u.nid_usuario::text,  
                   p.spersona_nombre,  
                   'USUARIOS'  
            from usuarios u  
    ...
    if 'equipos' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                t.nid_team,  
                CONCAT(  
    ...
    if 'supervisores_todos' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                NULL AS scodigo,  
                'AUTOGESTIONADO' AS sdescripcion,  
                'SUPERVISORES_TODOS' AS stipo_configuracion  
  
            UNION  
  
            SELECT                c.nId_Colaborador::text AS scodigo,  
    ...
    if 'supervisores_asignados' = any(var_configuration_request) then  
        RETURN QUERY            SELECT sa.nid_supervisor::text,  
    ...
    if 'equipos_lider' = any(var_configuration_request) then  
        RETURN QUERY            SELECT t.nid_team::text,  
                   CONCAT(  
                           p_lider.sprimer_nombre, ' ',  
                           p_lider.sape_paterno, ', ',  
                           p_director.sprimer_nombre, ' ',  
    ...
    if 'equipos_supervisor' = any(var_configuration_request) then  
        RETURN QUERY            SELECT DISTINCT CONCAT(pry.nid_lider, '/', pry.nid_director),  
    ...
    if 'fechas_cierre_asistencia' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                t.scodigo::text,  
                t.sdescripcion,  
    ...
    if 'metodologias' = any(var_configuration_request) then  
        RETURN QUERY            SELECT m.nid_metodologia::text as scodigo,  
    ...
    if 'categorias_solicitudes' = any(var_configuration_request) then  
        RETURN QUERY            SELECT cp.nid_categoria::text   AS scodigo,  
    ...
    if 'revision' = any(var_configuration_request) then  
        RETURN QUERY            SELECT (select sfiltros  
                    from revisiones  
    ...
    if 'solicitudes_usuario' = any(var_configuration_request) then  
        RETURN QUERY            SELECT s.nid_solictud::text,  
    ...
    if 'solicitud_por_dias_mes' = any(var_configuration_request) then  
        RETURN QUERY            SELECT CASE                       WHEN COUNT(s.nid_solictud) > 0 THEN '1'  
                       ELSE '0'  
                    END,  
                   'SOLICTID_POR_DIAS',  
                   'SOLICTID_POR_DIAS'  
    ...
    if 'proyectos_lider' = any(var_configuration_request) then  
        RETURN QUERY            SELECT pl.nid_lider::text,  
    ...
    if 'requerimiento_supervisor' = any(var_configuration_request) then  
        RETURN QUERY            select r.nid_requerimiento::text,  
            ...
            from requerimientos r  
                ...
                -- join Coordinadores_Proyectos cp on cp.nId_Coordinador = c.nId_Coordinador  
                -- JOIN Personas p ON c.nId_Persona = p.nId_Persona                     join supervisor_proyecto sp on sp.nid_proyecto = cp.nid_proyecto  
                     join coordinadores supervisor on supervisor.nid_coordinador = sp.nid_supervisor  
                     join personas p_sup on p_sup.nid_persona = supervisor.nid_persona  
                     join usuarios u_sup on u_sup.nid_persona = p_sup.nid_persona  
            WHERE u_sup.nid_usuario = p_user_id  
    ...
    if 'requerimiento_coordinador' = any(var_configuration_request) then  
        RETURN QUERY            select r.nid_requerimiento::text,  
            ...
            JOIN coordinadores_proyectos cp ON cp.nid_proyecto = pry.nid_proyecto  
                -- join Coordinadores_Proyectos cp on cp.nId_Coordinador = c.nId_Coordinador  
                -- JOIN Personas p ON c.nId_Persona = p.nId_Persona                -- join Supervisor_Proyecto sp on sp.nId_Proyecto = cp.nId_Proyecto            JOIN coordinadores coor on coor.nid_coordinador = cp.nid_coordinador  
    ...
    if 'rubros' = any(var_configuration_request) then  
        RETURN QUERY            SELECT r.nid_rubro::text,  
                   r.sdescripcion,  
                   'RUBRO'  
            FROM rubros r  
            WHERE r.nestado = 1;  
    ...
    /*if 'empresas_suggest' = any(var_configuration_request) then  
        RETURN QUERY            SELECT e.nid_empresa     AS sCodigo,                   e.snombre_empresa AS sDescripcion,                   'EMPRESAS'        AS sTipo_Configuracion            FROM empresas e;    end if;*/  
    ...
    if 'client_company_suggest' = any(var_configuration_request) then  
        RETURN QUERY            SELECT nid_cliente::text,  
                   snombre_cliente,  
                   'COMPANY'  
    ...
    if 'emails_list' = any(var_configuration_request) then  
        RETURN QUERY            SELECT nid_usuario::text,  
                   semail,  
                   'CORREO'  
            FROM Usuarios  
            WHERE nEstado = 1;  
    ...
    if 'solicitudes_pendientes' = any(var_configuration_request) then  
        RETURN QUERY            SELECT count(s.nid_solictud)::text,  
                   null,  
    ...
    if 'proyectos_total' = any(var_configuration_request) then  
        RETURN QUERY            SELECT COUNT(p.nid_proyecto)::text,  
                   null,  
                   'PROYECTOS_TOTAL'  
            FROM proyectos p;  
    end if;  
    ...
    if 'plantillas_contratos' = any(var_configuration_request) then  
        RETURN QUERY            SELECT CONCAT(ct.nempresa_usuaria, '-', ct.stipo_contratacion),  
                   ct.sid_drivefile,  
                   'TEMPLATES'  
    ...
    if 'cierre-tareo' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                nid_cierre::text,  
                dfecha_cierre::text,  
                'CIERRE_TAREO'  
            FROM cierre_horas  
    ...
    if 'categorias_all' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                nid_categoria::text,  
                snombre,  
                'CATEGORIAS_ALL'  
            FROM  
                categorias  
    ...
    if 'aprobadores' = any(var_configuration_request) then  
        RETURN QUERY            SELECT DISTINCT                nid_aprobador::text,  
                snombre_aprobador,  
    ...
    if 'remitente' = any(var_configuration_request) then  
        RETURN QUERY            SELECT  DISTINCT                nid_usuario::text,  
                semail,  
                'REMITENTE'  
            from usuarios  
  
            UNION  
            SELECT  DISTINCT                scodigo,  
                sdescripcion,  
                'REMITENTE'  
    ...
    if 'mailing_creator' = any(var_configuration_request) then  
        RETURN QUERY            select                distinct                mt.nusuario_creador::text,  
    ...
    if 'mailing_list' = any(var_configuration_request) then  
        RETURN QUERY            select                nid_mailingtemplate::text,  
                sname,  
                'MAILING_LIST'  
    ...
    if 'evaluation_template' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                nid_form_template::text,  
                sname,  
                'EVALUATION_TEMPLATE'  
            FROM form_template;  
    end if;  
    ...
    if 'evaluation' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                nid_evaluation::text,  
                sname,  
                'EVALUATION'  
            FROM evaluations;  
    end if;  
    ...
    if 'my_evaluations' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                e.nid_evaluation::text,  
                e.sname,  
                'MY_EVALUATIONS'  
            FROM (  
                     SELECT DISTINCT  
    ...
    if 'forms' = any(var_configuration_request) then  
        RETURN QUERY            SELECT                ft.nid_form_type::text,  
                ft.sname,  
                'FORMS'  
            FROM form_type ft  
    ...
    if 'planned_closing' = any(var_configuration_request) then  
        RETURN QUERY            SELECT              car.nid_cierre::text,  

...
--PROYECTOS LISTADO RESPONSIVE
    ...
    IF (SELECT COUNT(sConfiguration) from @configurations_request where sConfiguration = 'evaluation') = 1
        BEGIN
            ...
            FROM Evaluations
        END
    IF (SELECT COUNT(sConfiguration) from @configurations_request where sConfiguration = 'form_state') = 1
        BEGIN
            INSERT INTO @configuraciones
            SELECT
                sCodigo sCodigo,
                sDescripcion sDecripcion,
                'FORM_STATE' sTipo_Configuration
            FROM configs
            WHERE sTabla like('%ESTADO_FORMULARIO%')
        END
    IF (SELECT COUNT(sConfiguration) from @configurations_request where sConfiguration = 'my_evaluations') = 1
        BEGIN
            INSERT INTO @configuraciones
            SELECT
                e.nId_Evaluation sCodigo,
                e.sName sDecripcion,
                'MY_EVALUATIONS' sTipo_Configuration
            FROM (
                     SELECT DISTINCT
                         e.nId_Evaluation,
                         e.sName,
                         CASE
                             ...
                             WHEN CAST(DATEADD(HOUR, -5, GETDATE()) AS DATE) BETWEEN e.dInit_Date AND e.dEnd_Date THEN 2
                             WHEN CAST(DATEADD(HOUR, -5, GETDATE()) AS DATE) < e.dInit_Date THEN 4
                             WHEN CAST(DATEADD(HOUR, -5, GETDATE()) AS DATE) > e.dEnd_Date THEN 3
                     ...
                     WHERE ev.nId_Evaluator = @Id_Colaborador

                 ) e
            WHERE e.nState NOT IN (1, 4)
            ORDER BY e.nId_Evaluation DESC
        END
    IF (SELECT COUNT(sConfiguration) from @configurations_request where sConfiguration = 'forms') = 1
        BEGIN
            INSERT INTO @configuraciones
            SELECT
	...
	IF (SELECT COUNT(sConfiguration) from @configurations_request where sConfiguration = 'my_evaluations_completed') = 1
        BEGIN
            ...
            SELECT
                e.nId_Evaluation sCodigo,
                e.sName sDecripcion,
                'MY_EVALUATIONS_COMPLETED' sTipo_Configuration
            FROM (
                     SELECT DISTINCT
                         e.nId_Evaluation,
                         e.sName,
            ...
            WHERE e.nState = 3 and e.dShare_Results IS NOT NULL and e.dShare_Results <= DATEADD(HOUR, -5, GETDATE()) 
            ORDER BY e.nId_Evaluation DESC
    	END
     IF (SELECT COUNT(sConfiguration) from @configurations_request where sConfiguration = 'planned_closing') = 1
        BEGIN
            INSERT INTO @configuraciones
        	SELECT TOP 1
			    car.nId_Cierre AS sCodigo,

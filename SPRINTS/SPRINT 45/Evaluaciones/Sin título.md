...
AS
	WITH Evaluators_Completed as (
		...
		SELECT
		    ...
		    CAST(er.Answered AS FLOAT) / er.Expected AS Progress,
		    (
		        SELECT 
		            f2.nId_Form_Type,
		            COUNT(CASE WHEN ec.Answered = 1 THEN 1 END) AS Completed,
		            COUNT(CASE WHEN ec.Answered = 0 THEN 1 END) AS Pending,
		            COUNT(CASE WHEN ec.Answered = 0 THEN 1 END) + COUNT(CASE WHEN ec.Answered = 1 THEN 1 END) AS Total
		        FROM Evaluators_Completed ec
		        JOIN Forms f2 ON ec.nId_Form = f2.nId_Form
		        WHERE ec.nId_Evaluation = e.nId_Evaluation
		        GROUP BY f2.nId_Form_Type   
		        ORDER BY f2.nId_Form_Type   
		        FOR JSON PATH
		    ) AS FormTypes
		FROM Evaluations e

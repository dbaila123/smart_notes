...
CREATE OR ALTER  PROCEDURE [dbo].[sp_Update_LLenado_Tareo_Asistencia]  
(  
...
BEGIN  
 ...
 DECLARE @Tardanza_colaborador int = (SELECT TOP(1) dTiempo_Tardanza FROM Asistencias where dFecha_Asistencia = @dFecha_Asistencia and nId_Colaborador = @IdColaboradorIndividual)	
 ...
 DECLARE @HoraSalida time  
  ...
  set @Requerido = ((SELECT nRequerido FROM (SELECT  
         CASE  
...
-- LEFT JOIN v_horario_dias_total_horas vhdt ON vhdt.nId_Horario = hc.nId_Horario  
 LEFT JOIN ( SELECT  
    s.nId_Colaborador,  
       SUM(CASE  
      ...
      -- JOIN Horarios_Colaboradores hc on hc.nId_Colaborador = s.nId_Colaborador and hc.nEstado = 1  
 ...
 WHERE vhdt.nId_Horario = @nId_Horario and vhdt.nDia = @dDia_de_la_semana--I = @nId_Colaborador and vhdt.nDia = @dDia_de_la_semana and hc.nEstado = 1  
      )  
    ELSE  
    0  
         END nRequerido ) TEMP_REQUERIDO ) - @Tardanza_colaborador)
          ...
          CASE  
           ...
           WHEN (  
             (  
               SELECT COUNT(*)  
               FROM Solicitudes s  
               JOIN Tipos_Solicitudes ts ON ts.nId_Tipo_Solicitud = s.nId_Tipo_Solicitud                 WHERE s.nEstado_Solicitud in (1,3)  
               AND s.nId_Tipo_Solicitud NOT IN (21)  
               AND @dFecha_Asistencia BETWEEN s.dFecha_Inicio AND s.dFecha_Fin  
               AND ts.nAfecta_Asistencia = 1  
               AND (ts.nTipo_Frecuencia = 2 or ts.nTipo_Frecuencia = 3)  
  ...
  set @MinutosSolicitud =  (SELECT nMinutosPermiso FROM (SELECT  
          CASE  
           WHEN (SELECT COUNT(*) from v_horario_dias_total_horas s2  
            --JOIN Horarios_Colaboradores hc2 on hc2.nId_Colaborador = @nId_Colaborador  
            WHERE s2.nId_Horario = @nId_Horario and s2.nDia = @dDia_de_la_semana ) =0  
         ...
         -- JOIN Horarios_Colaboradores hc2 on hc2.nId_Colaborador = @nId_Colaborador  
          WHERE s2.nId_Horario = @nId_Horario  and s2.nDia = @dDia_de_la_semana  
           ...
           ELSE  
            0  
          END nMinutosPermiso ) TEMP_MINUTOS_SOLICITUD)  
  
  UPDATE Asistencias  
  SET nLlenado = @Llenado, nRequerido = @Requerido, nLlenadoNo_Regular = @nLlenadoNo_Regular, nRequerido_No_Regular = @nRequerido_No_Regular, nTiene_Marca = @TieneMarca, sEstadoLLenadoTareo = @EstadoLlenadoTareo ,  
   sEstadoTareoHorario = @EstadoHorarioTareo, nMinutos_Permiso = @MinutosSolicitud,  
   tHoraEntrada = @HoraEntrada , tHoraSalida = @HoraSalida  
  WHERE nId_Colaborador = @nId_Colaborador  

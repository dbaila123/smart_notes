...
BEGIN    
 ...
 ELSE IF @sSlugToGetList = 'INDICADORES-MODULO-COLABORADORES-EQUIPOS'    
    BEGIN    
        ...
        SET @nId_Coll = (SELECT c.nId_Colaborador FROM Colaboradores c    
        ...
        INSERT INTO @conditions VALUES ('mec.nId_Colaborador IN (' + @CollaboratorsList + ')');    
    END    
   ELSE IF @sSlugToGetList = 'INDICADORES-MODULO'    
    BEGIN    
        SET @nId_Coll = (SELECT c.nId_Colaborador FROM Colaboradores c    
                                JOIN Usuarios u ON u.nId_Persona = c.nId_Persona    
                                WHERE u.nId_Usuario = @nId_User);  
        INSERT INTO @conditions VALUES ('mec.nId_Colaborador IN (' + CONVERT(NVARCHAR(MAX), @nId_Coll) + ')');    
    END    

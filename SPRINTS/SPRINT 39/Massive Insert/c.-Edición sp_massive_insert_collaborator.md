...
null,                                          
   ...
   0,                              
   lower(dbo.fn_quitar_tildes(CASE                       
    WHEN EXISTS (SELECT 1 FROM Emails WHERE sEmail = @EmailEmpresarial)                          
     THEN                              
      CASE                                
       WHEN EXISTS (SELECT 1 FROM Emails WHERE sEmail = @EmailAlternativo)                                    
        THEN                                       
         CASE                         
                       WHEN EXISTS (SELECT 1 FROM Emails WHERE sEmail = @EmailAlternativoApellidoMaterno)                           
           THEN @EmailAlternativoApellidoMaternoContador                                          
            ELSE @EmailAlternativoApellidoMaterno                                   
           END                                
        ELSE @EmailAlternativo                              
